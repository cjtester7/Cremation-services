...
// Location: netlify/functions/send-email-v1.js
// CR012 — Handles 3 email types:
//   type: 'lead_notification'  — internal alert to Carewell staff on lead capture
//   type: 'visitor_confirmation' — warm acknowledgment to visitor on lead capture
//   type: 'transcript'         — conversation transcript to visitor-provided email
...
function buildTranscriptEmail(to_email, transcript_text, filename) {
  ...
  const htmlLines = lines.map(line => {
    if (line.startsWith('=') || line.startsWith('-')) return `<hr style="border:none;border-top:1px solid #e8e4de;margin:0.5rem 0;"/>`;
    if (line.trim() === '') return '<br/>';
    if (line.startsWith('[')) return `<p style="margin:0.1rem 0;font-size:0.82rem;color:#6b6b6b;">${line}</p>`;
    if (line.startsWith('  CAREWELL')) return `<p style="margin:0.25rem 0;font-weight:600;color:#2d4a35;font-size:0.9rem;">${line.trim()}</p>`;
    return `<p style="margin:0.1rem 0;font-size:0.875rem;line-height:1.55;">${line}</p>`;
  }).join('');

  const html = `
    <div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;color:#2c2c2c;">
      <div style="background:linear-gradient(135deg,#2d4a35,#3d5c45);padding:1.5rem 2rem;border-radius:0.75rem 0.75rem 0 0;">
      ...
      <div style="background:#fff;padding:1.5rem 2rem;border:1px solid #e8e4de;border-top:none;font-family:monospace,monospace;">
        ${htmlLines}
      </div>
      <div style="background:#f8f5f0;padding:1rem 2rem;border:1px solid #e8e4de;border-top:none;border-radius:0 0 0.75rem 0.75rem;font-size:0.8rem;color:#6b6b6b;">
        To speak with our team: <a href="tel:5713002273" style="color:#5a7a64;">571-300-2273</a> · Available 24/7<br/>
        <a href="https://calendly.com/cjtester7/free-30-minute-consultation" style="color:#5a7a64;">Schedule a free consultation</a>
      </div>
    </div>`;
...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.error('send-email-v1 error:', err);
